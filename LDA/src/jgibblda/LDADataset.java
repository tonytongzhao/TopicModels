...
public class LDADataset {
	...
	// Instance Variables
	//---------------------------------------------------------------
	public HashMap<Integer, String> id2concept;
	public Dictionary localDict;
	public Dictionary userDict;			// local dictionary	 user dictionary
	...
	public LDADataset(){
		...
		V = 0;
		docs = null;
		id2concept=new HashMap<Integer, String>();
		globalDict = null;
		lid2gid = null;
	}
	...
	public  void  readAuthorSet(LDADataset data,LDACmdOption option, BufferedReader reader)
	{
		int i=0;
		String tmp0=null;
		...
		try {
			...
			while((tmp0=reader.readLine())!=null)
			{
				String [] tmp=tmp0.split("\t");
				data.docs[i].authors=new int[tmp.length-1];
				for (int j=1;j<tmp.length;j++){
					int _id = userDict.word2id.size();
					if (userDict.contains(tmp[j]))		
						{
							_id = userDict.getID(tmp[j]);
						}
					else
					{
						userDict.addWord(tmp[j]);
					}
					data.docs[i].authors[j-1]=_id;
					X=userDict.word2id.size();
				}
			}
		} catch (IOException e) {
	...
	public static LDADataset readDataSet(String filename,LDACmdOption option){
		try {
			...
			reader.close();
			//BufferedReader reader1 = new BufferedReader(new FileReader(option.dir+File.separator+option.dfile));
			//data.readAuthorSet(data, option, reader1);			
			//reader1.close();
			reader=new BufferedReader(new FileReader(new File(option.dir+File.separator+"concept.txt")));
			String line;
			while((line=reader.readLine())!=null)
			{
				data.id2concept.put(Integer.parseInt(line.split(";")[0]), line.split(";")[1]);
			}
			reader.close();
			return data;
		}
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSet(String filename, Dictionary dict,LDACmdOption option){
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(
					new FileInputStream(filename), "UTF-8"));
			LDADataset data = readDataSet(reader, dict,option.dir);
	...
	public static LDADataset readDataSet(BufferedReader reader, String dir){
		try {
			...
			//read each document name of training dataset
			LDADataset data = new LDADataset(M);
			for (int i = 0; i < M; ++i){
				filename = reader.readLine().split(";")[1];
			//	System.out.println(line);
				BufferedReader br=new BufferedReader(new FileReader(dir+File.separator+filename+".txt"));
				String tmp;
				line=null;
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSet(BufferedReader reader, Dictionary dict, String dir){
		try {
			//read number of document
