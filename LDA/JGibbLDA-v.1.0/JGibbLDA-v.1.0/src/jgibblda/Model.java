...
public class Model {	
	 ...
	 * @throws Exception 
	 */
	public boolean initNewModel(LDACmdOption option) throws Exception{
		if (!init(option))
			return false;
		//BufferedWriter bw =new BufferedWriter(new FileWriter(new File("D:\\Datasets\\TwitterDataset\\2010_10_14\\FinalProcess\\TopicModelwithStanford\\SentimentClasswithStanford.txt")));
		int m, n, w, k;		
		p = new double[K];		
		...
		for (m = 0; m < data.M; m++){
			...
			//initilize for z
			for (n = 0; n < N; n++){
				//bw.write(" "+data.docs[m].words[n]+":1");
