...
public class Model {    
        ...
        public int S=3;  // 0 negative; 1 positive; 
                    	...
                    	for (i = 0; i < M; i++) {
            				...
            				for (j = 0; j < length; j++) {
            					...
            					if (tmp.contains(",")) {
            						...
            						zl[i].add(Integer.parseInt(tmp1[1]));
                ...
                try{
                        ...
                        for (i = 0; i < data.M; i++){
                                for (j = 0; j < data.docs[i].length; ++j){
                                        writer.write(data.docs[i].words[j] + ":" +za[i].get(j) +","+zl[i].get(j)+","+z[i].get(j) + " ");                                  
                                }
                                writer.write("\n");
                        }
        ...
        /*public boolean saveModelThetaDoc(String filename){
            try{
                  ...
                  writer=new BufferedWriter(new FileWriter("Author_Doc_Positive.txt"));
                  for (int a = 0; a < A; a++){
                	  ...
                	  List<Pair> TopicProbsList = new ArrayList<Pair>(); 
                      	for(int k=0;k<K;k++)
                      	{
                      		 Pair p = new Pair(k, theta[a][k][1], false);                            
                      ...
                      for(int i=0;i<3;i++)
                      {
                          ...
                          for (int d = 0; d < M; d++){
                                  Pair p = new Pair(d, theta_doc[d][Integer.parseInt(TopicProbsList.get(i).first.toString())][1], false);
                          ...
                          for(int j=0;j<100;j++)
                          {
                        	  writer.write(DocProbsList.get(j).first+"\n");
                          }
                      }
                  ...
                  writer=new BufferedWriter(new FileWriter("Author_Doc_Negative.txt"));
                  for (int a = 0; a < A; a++){
                	  	writer.write("Author: "+a+"\n");
                	  //writer.write("Topic "+i+"th\n");
                	  List<Pair> TopicProbsList = new ArrayList<Pair>(); 
                      	for(int k=0;k<K;k++)
                      	{
                      		 Pair p = new Pair(k, theta[a][k][0], false);                            
                             TopicProbsList.add(p);
                      	}
                      Collections.sort(TopicProbsList);            
                      for(int i=0;i<3;i++)
                      {
                    	  writer.write("Topic "+TopicProbsList.get(i).first.toString()+"th\n");
                    	  System.out.println("Topic "+TopicProbsList.get(i).first.toString()+"th "+TopicProbsList.get(i).second.toString());
                    	  List<Pair> DocProbsList = new ArrayList<Pair>(); 
                          for (int d = 0; d < M; d++){
                                  Pair p = new Pair(d, theta_doc[d][Integer.parseInt(TopicProbsList.get(i).first.toString())][0], false);
                  ...
                  writer=new BufferedWriter(new FileWriter("Topic_Author_Positive.txt"));
                  for (int k = 0; k < K; k++){
                	  	writer.write("Topic: "+k+"\n");
                	  //writer.write("Topic "+i+"th\n");
                	  List<Pair> AuthorProbsList = new ArrayList<Pair>(); 
                      	for(int a=0;a<A;a++)
                      	{
                      		 Pair p = new Pair(a, theta[a][k][1], false);                            
                      		AuthorProbsList.add(p);
                      	}
                      Collections.sort(AuthorProbsList);            
                      for(int i=0;i<3;i++)
                      {
                    	  writer.write("Author " +AuthorProbsList.get(i).first.toString()+" "+AuthorProbsList.get(i).second +"\n");
                    	  List<Pair> DocProbsList = new ArrayList<Pair>(); 
                          for (int d = 0; d < M; d++){
                                  Pair p = new Pair(d, theta_doc[d][k][1], false);
                  ...
                  writer.close();
                  writer=new BufferedWriter(new FileWriter("Topic_Author_Negative.txt"));
                  for (int k = 0; k < K; k++){
                	  	writer.write("Topic: "+k+"\n");
                	  //writer.write("Topic "+i+"th\n");
                	  List<Pair> AuthorProbsList = new ArrayList<Pair>(); 
                      	for(int a=0;a<A;a++)
                      	{
                      		 Pair p = new Pair(a, theta[a][k][0], false);                            
                      		AuthorProbsList.add(p);
                      	}
                      Collections.sort(AuthorProbsList);            
                      for(int i=0;i<3;i++)
                      {
                    	  ...
                    	  List<Pair> DocProbsList = new ArrayList<Pair>(); 
                          for (int d = 0; d < M; d++){
                                  Pair p = new Pair(d, theta_doc[d][k][0], false);
            ...
            // niters, savestep: from command line or default values
                ...
                for (m = 0; m < data.M; m++){
                        ...
                        for (n = 0; n < N; n++){
                                ...
                                int a = da[m].get((int) (Math.random() * num_authors));
                                int topic = (int)Math.floor(Math.random() * K);
                                int senti_label=(int)Math.floor(Math.random() * S);
        ...
        public boolean initEstimatedModel(LDACmdOption option){
            ...
            for (m = 0; m < data.M; m++){
                ...
                for (n = 0; n < N; n++){
                        ...
                        int senti=(Integer)zl[m].get(n);
                       
                        nasum[a]+=1;                   
                        nwl[data.docs[m].words[n]][topic][senti] += 1 ;                                                        
                        nal[a][topic][senti]+= 1 ;                                               
                        nwlsum[topic][senti] += 1 ;                                  
                        nalsum[a][senti] += 1 ;        
                }
