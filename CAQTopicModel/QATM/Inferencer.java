...
public class Inferencer {       
        ...
        protected int infSampling(int m, int n){
                ...
                newModel.nqat[m][_replier][topic] -= 1;
                newModel.nqa[m][_replier]-=1;
                newModel.nwsum[topic] -= 1;
                newModel.nqatsum[m][_replier]-=1;
                newModel.nqasum[m] -= 1;
                ...
                for (int b = 0; b < num_authors; ++b)
                {
                        ...
                        for (int k = 0; k < trnModel.K; k++){
                                ps[current_index] = (trnModel.nw[w][k]+newModel.nw[_w][k] + trnModel.beta)/(trnModel.nwsum[k] +newModel.nwsum[k]+ Vbeta) * (newModel.nqat[m][a][k] + newModel.alpha)/(newModel.nqatsum[m][a] + Kalpha)*(newModel.nqa[m][a] + newModel.gammaforusers[m][a])/(newModel.nqasum[m] + newModel.A*trnModel.gammaforusers[m][a]);
                ...
                newModel.nqasum[m] += 1;
                //* trnModel.word_weights[w][m];
                newModel.nqat[m][a][topic] += 1;
        		
                newModel.nqatsum[m][a] += 1;
                ...
                for (int m = 0; m < newModel.M; m++){
                	for (int n = 0; n < newModel.A; n++){
                        for (int k = 0; k < newModel.K; k++){
                                newModel.theta[m][n][k] = (newModel.nqat[m][n][k] + newModel.alpha) / (newModel.nqatsum[m][n] + newModel.K * newModel.alpha);
                        }//end foreach topic
                	}
        ...
        public void computeNewPi(){
            for (int a = 0; a < newModel.M; a++){
                    for (int b = 0; b < newModel.A; b++){
                    	newModel.pi[a][b] = (newModel.nqa[a][b] + newModel.gammaforusers[a][b]) / (newModel.nqasum[a] + newModel.A * newModel.gammaforusers[a][b]);
                    }
            }
        }
