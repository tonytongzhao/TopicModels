...
public class Model {
	...
	protected int[][] nqatsum;

	protected int[] nqasum;
	...
	public boolean initNewModel(LDACmdOption option) {
		...
		for (m = 0; m < M; m++) {
			...
			ARcount[m][bestanswerer] += 10;
			for (n = 2; n < da[m].size(); n++) {
				ARcount[m][da[m].get(n)] += 1;
			}
		}
		...
		for (m = 0; m < data.M; m++) {
			...
			prior[0] = gammaforusers[m][0];
			for (q = 1; q < A; q++) {
				prior[q] = prior[q - 1] + gammaforusers[m][q];
			}
			// initialize for z
			for (n = 0; n < N; n++) {
