...
public class Model {
	...
	public double[][] theta; // theta: asker-topic distributions, size A x K
	public double[][] phi; // phi: topic-word distributions, size K x V
	public double[][][] pi; // pi: asker-topic-responder distributions, size A x A x K
	public double[][][] gammaforaskertopic; //gamma for specific topic-asker pair A X A X K
	public double[][] gammatopicsum;
	public double[][][] ARTcount; // background matrix, size A x A x K
	public GBPair[][] GBCount;
	...
	protected int[][] nxz;
	protected int[][][] nxzr;
	protected int[] nzsumw;
	protected int[] nxsumz;
	protected int[][] nxzsumr;
	...
	// ***
	// DON'T USE - HAVE NOT UPDATED THIS METHOD FOR ATM
	// Load word-topic; word-asker, word-responder
	// ***
	...
	public boolean loadModel() {
		//System.out.println(dir + File.separator + modelName + othersSuffix);
		modelName = "model-final";
	...
	/**
	 * Save responder-asker-topic distribution
	 */
	public boolean saveModelPi(String filename) {
		try {
			...
			writer.close();
			writer=new BufferedWriter(new FileWriter(filename.replaceFirst(".pi", ".list")));
			writer.write(M+"\n");
			for (int k = 0; k < K; k++) {
				writer.write("Topic " + k + "\n");
				for (int m = 0; m < M; m++) {
					List<Pair> userlist = new ArrayList<Pair>();			
					int asker=data.docs[m].authors[0];
					for (int r = 1; r < data.A; r++) {				
						Pair p = new Pair(r, pi[asker][k][r], false);
						userlist.add(p);				
					}			
					Collections.sort(userlist);
					for (int i = 0; i < userlist.size()-1; i++) {
						writer.write( userlist.get(i).first + ",");
					}
					writer.write(userlist.get( userlist.size()-1).first+";"+data.docs[m].authors[1]+";");				
					for (int t = 1; t < data.docs[m].authors.length; t++) {
						writer.write(data.docs[m].authors[t] + ",");
					}
					writer.newLine();			
				} 
			}
	...
	public boolean initNewModel(LDACmdOption option) {
		...
		gammaforaskertopic = new double[A][A][K];
		for (m = 0; m < A; m++) {
			for (k=0; k < K; k++){
				for (n = 0; n < A; n++) {
					gammaforaskertopic[m][n][k] = 0;
				}
			}
		}

		ARTcount = new double[A][A][K];
		for (m = 0; m < A; m++) {
			for (n = 0; n < A; n++) {
				for (k = 0; k < K; k++)
					ARTcount[m][n][k] = 1.0;
		...
		for(k = 0; k < K; k++){
			for (m = 0; m < A; m++) {
				for (n = 0; n < A; n++) {
					gammaforaskertopic[m][n][k] = ARTcount[m][n][k] * 0.01;
					gammatopicsum[m][k] += gammaforaskertopic[m][n][k];
				}
			}
		}
		
		GBCount = new GBPair[A][K];
		for (m = 0; m < A; m++){
			for (k = 0; k < K; k++)
				GBCount[m][k] = new GBPair(1, 1);
		}
		// initialize da!
		...
		for (m = 0; m < data.M; m++) {
			...
			for (n = 0; n < N; n++) {
				...
				prior[0] = gammaforaskertopic[a][0][topic];
				for (q = 1; q < A; q++) {
					prior[q] = prior[q - 1] + gammaforaskertopic[a][q][topic];
					//System.out.println(prior[q]);
				}
				int r = multinomial(prior);
				z[m].add(topic);
				za[m].add(a);
				zr[m].add(r);
				nwz[data.docs[m].words[n]][topic] += 1;
				nxz[a][topic] += 1;
				nxzr[a][topic][r] += 1;
				nzsumw[topic] += 1;
				nxsumz[a] += 1;
		...
		{
			re+=data.docs[i].authors.length-1;
		}
		
		System.out.println((double)re/data.M);
		return true;
	}
	...
	public boolean initNewModel(LDACmdOption option, LDADataset newData,
		...
		for( k = 0; k < K; k++)
			for (m = 0; m < A; m++) {
				for (n = 0; n < A; n++) {		
					gammaforaskertopic[m][n][k] = gamma;
					gammatopicsum[m][k] += gammaforaskertopic[m][n][k];
				}
			}
		nzsumw = new int[K];
		...
		for (m = 0; m < data.M; m++) {
			...
			for (n = 0; n < N; n++) {
				...
				if (data.la2ga.get(a) != null && data.la2ga.get(r) != null) {
					gammaforaskertopic[a][r][topic] = trnModel.ARTcount[data.la2ga.get(a)][data.la2ga
							.get(r)][topic]* gammaforaskertopic[a][r][topic];
					gammatopicsum[a][topic]+=gammaforaskertopic[a][r][topic]-gamma;
				}
				nwz[data.docs[m].words[n]][topic] += 1;
				nxz[a][topic] += 1;
				nxzr[a][topic][r] += 1;
...
//		for( int k = 0; k < K; k++)
//			for (int m = 0; m < A; m++) {
//				gammatopicsum[m][k] = 0.0;
//				for (int n = 0; n < A; n++) {		
//					gammaforaskertopic[m][n][k] = 0.01*ARTcount[m][n][k];
//					gammatopicsum[m][k] += gammaforaskertopic[m][n][k];
//				}
//			}
//	}
