...
public class Model {
	...
	public double[][][] theta; // theta: author - topic distributions, size  A x A x K
	...
	public Vector<Integer>[] z; // topic assignments for words, size M x V

	// doc.size()

	public Vector<Integer>[] za; // author assignments for words, size 1
	
	public Vector<Integer>[] daskers;
	// doc.size()

	public Vector<Integer>[] zr;// responder assignments for words, x

	public Vector<Integer>[] dr; // responder assignments for doc 
...
*/
	...
	public boolean initNewModel(LDACmdOption option) {
		...
		//	}
		//}
		// initialize da! d_a d_r
		daskers=new Vector[M];
		da = new Vector[M];
		dr = new Vector[M];
		for (m = 0; m < data.M; m++) {
			daskers[m]=new Vector<Integer>();
			...
			if(!daskers[m].contains(data.docs[m].authors[0]))
			{
				daskers[m].add(data.docs[m].authors[0]);
			}
		...
		for (m = 0; m < data.M; m++) {
			int N = data.docs[m].length;//no. of words
